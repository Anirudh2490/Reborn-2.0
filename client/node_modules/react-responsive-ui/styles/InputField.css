...
.rrui__input-field__icon {
	position: absolute;
	top: calc(1.5 * var(--rrui-unit) / 2);
	left: 0;
	width: calc(var(--rrui-input-height) - 1.5 * var(--rrui-unit));
	height: calc(var(--rrui-input-height) - 1.5 * var(--rrui-unit));
	pointer-events: none;
	color: var(--rrui-gray-color);
}

.rrui__input-field--with-icon {
	padding-left: calc(var(--rrui-input-height) - 1.5 * var(--rrui-unit) / 2);
}